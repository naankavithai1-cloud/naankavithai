// =========================================================
// 12. admin.js - OWNER DASHBOARD LOGIC
// =========================================================
...
import { getFirestore, collection, query, where, orderBy, onSnapshot, doc, updateDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
function handleLoginRedirect() {
    window.location.href = 'index.html'; // Redirect to main page for Google Login
}
...
function checkAdminStatus(user) {
    if (user && user.uid === OWNER_UID) {
        ...
        setupTabListeners();
        setupPendingPoemsListener(); // Load default tab content
...
onAuthStateChanged(auth, (user) => {
    // This function ensures the page is only visible to the owner.
...
function setupTabListeners() {
    tabNavigation.addEventListener('click', (e) => {
        if (e.target.tagName === 'BUTTON') {
            ...
            // Activate current button and show target pane
...
/**
 * Renders a single pending poem card for approval.
 */
...
/**
 * Sets up real-time listener for pending poems.
 */
...
async function updatePoemStatus(poemId, status) {
    try {
        ...
        // If approved, trigger Cloud Function to update totalPosts count on author's profile (Future)
...
function renderUserRow(user) {
    ...
    row.innerHTML = `
        <div class="item-content">
            <img src="${user.photoURL || 'https://via.placeholder.com/30/0A0A0A/FFD700?text=U'}" alt="${user.name}" class="comment-avatar">
            <div>
                <p><strong>${user.name}</strong> (${user.uid.substring(0, 5)}...)</p>
                <small>Posts: ${user.totalPosts || 0} | Points: ${user.earningsPoints || 0}</small>
                <span class="${statusClass}">${user.isBanned ? 'BANNED' : 'Active'}</span>
            </div>
        </div>
        <div class="item-actions">
...
/**
 * Sets up real-time listener for all users (excluding admin for simplicity).
 */
...
/**
 * Renders a single withdrawal request row.
 */
...
/**
 * Sets up real-time listener for withdrawal requests.
 */
...
/**
 * Marks a withdrawal request as paid (Manual Payout System).
 */
async function markRequestAsPaid(requestId) {
    // IMPORTANT: The actual money transfer must happen OUTSIDE this system.
    ...
    if (confirmPaid) {
        try {
            ...
            await updateDoc(requestRef, {
                ...
                processedBy: auth.currentUser.uid, // Track who processed it
            });
...
document.addEventListener('DOMContentLoaded', () => {
    setCurrentYear();
    // checkAdminStatus is the primary loader via onAuthStateChanged
});
