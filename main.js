...
// All critical login, authentication state, and form functions are finalized here.
// =========================================================
...
// =========================================================
// 3. UI ELEMENT REFERENCES (Must be defined globally)
// =========================================================
...
// =========================================================
// 4. AUTHENTICATION LOGIC (Login, Logout, State Management)
// =========================================================

/**
 * Handles Google Sign-In and triggers a reload after successful data save.
 */
...
/**
 * Saves or updates essential user data, including the critical 'isAdmin' flag.
 */
...
/**
 * Handles user sign-out and triggers a page reload.
 */
...
/**
 * Listens for user state changes and updates the Header/Nav UI accordingly.
 */
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        // Admin Link Visibility (Relies on saveAtomicUserDetails setting the flag correctly)
        ...
        // Update/Save user details in the background
        saveAtomicUserDetails(user).catch(err => console.error("Error updating user details:", err));

    } else {
        // Logged Out: Show Login, Hide Profile/Admin Link
...
/**
 * Handles the poem form submission, image upload, and Firestore save.
 */
...
/**
 * Renders a single poem card for the main feed.
 */
...
/**
 * Sets up the real-time listener for APPROVED poems on the home feed.
 */
function setupRealtimeFeed() {
    const q = query(
        collection(db, "poems"),
        where("status", "==", "APPROVED"),
        orderBy("createdAt", "desc"), // Ensure you have the status/createdAt index for this query!
        limit(20) 
    );
